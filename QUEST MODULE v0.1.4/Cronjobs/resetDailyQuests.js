// =====================================
// FILE 6: resetDailyQuest.js (Cron Job - Daily at midnight)
// =====================================
            ...
            for (const variable of allModuleVars.data.data) {
                try {
                    ...
                    if (key.includes('dailyquest_') || key.includes('session_')) {
                        ...
                        // Check if it's an old quest
                        if (keyParts.length >= 6) {
                            const dateStr = `${keyParts[2]} ${keyParts[3]} ${keyParts[4]} ${keyParts[5]}`;
                            const variableDate = new Date(dateStr);

                            if (isNaN(variableDate.getTime()) || variableDate < cutoffDate) {
                                shouldDelete = true;
                            }
                        } else if (!key.includes(today)) {
                            // If key doesn't contain today's date, it's old
                            shouldDelete = true;
                        }
